...
from models.FlipClassroom import FlipClassEnsemble
...
class Trainer(object):
    ...
    def _create_model(self):
        if self.args.model_name == "fcrllm_sasrec":
            self.model = FlipClassEnsemble(self.user_num, self.item_num, self.device, self.args)
        elif self.args.model_name == "fcrllm_gru4rec":
            self.model = FlipClassEnsemble(self.user_num, self.item_num, self.device, self.args)
        else:
            raise ValueError
