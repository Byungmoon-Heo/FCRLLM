...
def hopfield_update_full(
    teacherK, studentQ,
    beta=1.0, alpha=0.1, gamma_update=0.5,
    diag_reg=True,
    pad_mask=None,   
    eps=1e-8
):
"""
Per-sequence Hopfield update with optional PAD masking (no cross-batch interactions).
To use this variant, rename this file to `FlipClassroom.py` (replacing the original) before running.
"""
    B, L, d = teacherK.shape

    scores = torch.matmul(teacherK, studentQ.transpose(1, 2))

    if pad_mask is not None:
        
        valid_cols = (~pad_mask).unsqueeze(1)
        scores = scores.masked_fill(~valid_cols, float("-inf"))

        
        all_cols_masked = (valid_cols.sum(-1) == 0)
        if all_cols_masked.any():
            scores[all_cols_masked.expand(-1, scores.size(1))] = 0.0

    attn = F.softmax(beta * scores, dim=-1)
    align = torch.matmul(attn, studentQ)   

    reg = alpha * teacherK                 

    if diag_reg:
        row_norm = torch.sum(teacherK * teacherK, dim=-1)  
        row_sm = F.softmax(0.5 * row_norm, dim=-1)      
        diag = row_sm.unsqueeze(-1) * teacherK         
    else:
        diag = torch.zeros_like(teacherK)

    delta = (align - reg - diag)         

    if pad_mask is not None:
        delta = delta.masked_fill(pad_mask.unsqueeze(-1), 0.0)

    updated = teacherK + gamma_update * delta
    return updated
...
class FlipBlockST(nn.Module):
    ...
    def forward(self, collab, semantic, pad_mask=None):
        s, t = collab, semantic
        for layer_s, layer_t in zip(self.student_layers, self.teacher_layers):
            s = layer_s(s)
            t = layer_t(t)
        t_upd = hopfield_update_full(t, s, pad_mask=pad_mask, **self.cfg)
        return s, t_upd
...
class FlipModelST(_BaseFlip):
    ...
    def forward(self, seq, pos, neg, positions, **kw):
        ...
        stud, teach_upd = self.block(co, se, pad_mask=pad_mask)

        diff2 = (stud - teach_upd).pow(2).sum(dim=-1)          
        valid = (~pad_mask).float()
        align_loss = (diff2 * valid).sum() / (valid.sum() + 1e-8)
...
class FlipModelCT(_BaseFlip):
    ...
    def forward(self, seq, pos, neg, positions, **kw):
        ...
        teach_upd, stud = self.block(co, se, pad_mask=pad_mask)

        diff2 = (teach_upd - stud).pow(2).sum(dim=-1)        
        valid = (~pad_mask).float()
        align_loss = (diff2 * valid).sum() / (valid.sum() + 1e-8)
